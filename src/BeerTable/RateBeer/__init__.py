...
from BeerGrabber.ratebeer import RateBeerGrabber
...
class BeerTable(QTableView):
    def __init__(self, model, beergraber, parent=None):
        ...
        self.origFields = OrderedDict([(0, 'brewery'), 
            (1, 'beer'), (2, 'ABV'), (3, 'Added'), (4, 'Score'), (5, 'Style'), 
            (6, 'Reviews'), (7, 'RATINGS:'), (8, 'MEAN:'), (9, 'WEIGHTED AVG:'), 
            (10, 'SEASONAL:'), (11, 'EST. CALORIES:'), (12, 'ABV:'), (13, 'IBU:'), 
            (14, 'IMAGE'), (-1, 'Id')])
        self.tableFields = [-1, 0, 1, 2, 12, 3, 4, 5, 6, 7, 8, 9, 10, 11, 13]
        self.loadBeers()
    ...
    def loadBeers(self):
        ...
        rowId = -1
        for row in rows:
            row[2] = self.toNumber(row[2])
            row[3] = self.toDate(row[3])
            row[4] = self.toNumber(row[4])
            row[6] = self.toInteger(row[6])
            row[7] = self.toInteger(row[7])
            if isinstance(row[8], str):
                row[8], _ = row[8].split(chr(47), 1)
                row[8] = self.toNumber(row[8])
            if isinstance(row[9], str):
                row[9], _ = row[9].split(chr(47), 1)
                row[9] = self.toNumber(row[9])
            row[11] = self.toInteger(row[11])
            row[12] = self.toNumber(row[12])
            row[13] = self.toInteger(row[13])
            ...
            for index, image_path in enumerate([row[14]]):
...
if __name__ == '__main__':
    ...
    proxy = {}
    countries = []
    #path = os.path.join(os.getcwd(), 'RateBeer')
    path = os.path.join('..', '..', 'RateBeer')
    
    wnd = BeerTable(BeerModel(), 
        RateBeerGrabber(path, proxy, update_all=False, countries=countries))
    wnd.showMaximized()
    app.exec()
